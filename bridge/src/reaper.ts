...
function matchFingerprint(cmdline: string): string | null {
  for (const fp of ARGV_FINGERPRINTS) {
    if (cmdline.includes(fp)) return fp;
  }
  // Cross-card prompt format: "Card: <title> [card-<id>]"
  if (cmdline.includes('Card: ') && cmdline.includes('[card-')) return 'Card: ... [card-';
  return null;
}
...
export type SkipEntry =
  ...
  | { kind: 'pattern'; pattern: string; raw: string };

/** One entry per line; numeric line = PID, anything else = cmdline substring. `#` comments. */
export function parseSkipList(content: string): SkipEntry[] {
...
function matchSkip(entries: SkipEntry[], pid: number, cmdline: string): SkipEntry | null {
  for (const e of entries) {
    if (e.kind === 'pid' && e.pid === pid) return e;
    if (e.kind === 'pattern' && cmdline.includes(e.pattern)) return e;
  }
  return null;
}
...
export function evaluateCandidate(c: CandidateInfo, ctx: EvalContext): ReaperDecision {
  const spare = (reason: string, matchedProvenance = false): ReaperDecision =>
    ({ action: 'spare', reasons: [reason], matchedProvenance });
  ...
  if (fp) provenanceReasons.push(`argv fingerprint: ${fp}`);
...
// ---------------------------------------------------------------------------
// /proc readers (every read tolerates the process vanishing mid-scan)
// ---------------------------------------------------------------------------
...
function readSlycodeSession(pid: number): string | null {
  try {
    const raw = fs.readFileSync(`/proc/${pid}/environ`, 'utf-8');
    for (const entry of raw.split('\0')) {
      if (entry.startsWith('SLYCODE_SESSION=')) return entry.slice('SLYCODE_SESSION='.length);
    }
    return null;
  } catch {
...
let cachedBootTimeMs: number | null = null;

function readBootTimeMs(): number {
  if (cachedBootTimeMs !== null) return cachedBootTimeMs;
  try {
...
export class Reaper {
  ...
  private history: Map<string, SweepHistory> = new Map();
  ...
  start(): void {
    ...
    this.timer.unref();
    console.log(
      `[reaper] active: every ${this.config.intervalMinutes}min, idle threshold ${this.config.idleHours}h` +
  ...
  private async runSweep(): Promise<void> {
    ...
    try {
      ...
      console.error('[reaper] sweep failed:', (err as Error).message);
    } finally {
      this.sweeping = false;
    }
  }
  ...
  async sweep(nowMs: number = Date.now()): Promise<SweepEntry[]> {
    ...
    for (const pid of listProcPids()) {
      ...
      const hist: SweepHistory = { cpuTicks: stat.cpuTicks, quietSweeps, termSentAt: prev?.termSentAt };
      ...
      if (decision.action === 'kill') {
        if (this.config.dryRun) {
          ...
          try {
            ...
            if (!escalate) hist.termSentAt = nowMs;
          } catch {
  ...
  private readSkipList(): SkipEntry[] {
    try {
  ...
  private log(entry: SweepEntry, c: CandidateInfo, nowMs: number): void {
    const verb = entry.dryRun ? 'DRY-RUN would kill' : entry.signal ? `kill(${entry.signal})` : entry.action;
    const line =
      `${new Date(nowMs).toISOString()} [${verb}] pid=${entry.pid} comm=${c.comm} ` +
    ...
    try {
      ...
      console.error('[reaper] could not write log:', (err as Error).message);
    }
  }
}
...
export function resolveReaperPaths(): { logPath: string; skipFilePath: string } {
