...
import {
  ...
  chooseDetectionCandidate,
  assessAssignedIdVerification,
  GUID_REARM_COOLDOWN_MS,
  ASSIGNED_ID_VERIFY_COOLDOWN_MS,
...
export class SessionManager {
   ...
   */
  private detectSessionIdAtExit(name: string): Promise<void> {
  ...
  private async detectSessionIdAtExitBody(name: string, session: Session): Promise<void> {
   ...
   */
  private async claimDetectedSessionId(
    name: string,
    sessionId: string,
   ...
   */
  private async ensureSessionIdDetection(name: string): Promise<void> {
    const session = this.sessions.get(name);
    if (!session) return;
    ...
    try {
      ...
      if (live) live.guidDetectionInFlight = false;
    }
  }
   ...
   */
  private async verifyAssignedSessionId(name: string, session: Session): Promise<boolean> {
    const now = Date.now();
  ...
  private async performVerifiedDelivery(resolvedName: string, prompt: string): Promise<DeliveryResult> {
    ...
    let outputSinceEnter: boolean | undefined;
    ...
    const mk = (outcome: DeliveryResult['outcome'], extra: Partial<DeliveryResult> & { verified?: boolean; mode?: DeliveryResult['mode'] } = {}, attempts = 0, resends = 0): DeliveryResult => {
      if (outcome !== 'delivered' && extra.verified !== false) diag(`outcome_${outcome}`, { reason: extra.reason, attempts, resends });
      return {
        outcome,
...

    {
      ...
      let repasted = false;
      const confirmQueued = (): boolean => {
        if (post === 'queued_ours') return true;
      ...
      for (const extraDelay of SessionManager.POST_PASTE_CONFIRM_DELAYS_MS) {
        if (confirmQueued()) break;                        // settled + matched (composer or novelty)
      ...
      const maxResends = SessionManager.VERIFY_MAX_RESENDS_BY_PROVIDER[session.provider as SubmitProvider] ?? SessionManager.VERIFY_MAX_RESENDS;
      if (dropFirstEnter) {
      ...
      for (;;) {
        ...
        for (const delay of SessionManager.VERIFY_POLL_DELAYS_MS) {
          ...
          if (c === 'unrecognized' && noveltyConfirmed
            ...
            if (!warnings.includes('delivered_by_placeholder_novelty')) warnings.push('delivered_by_placeholder_novelty');
            c = 'empty';
          }
          ladder.push(c);
          pollsLog.push(c);
