...
export function createApiRouter(sessionManager: SessionManager, responseStore: ResponseStore): Router {
  ...
  router.post('/sessions/:name/stop', async (req, res) => {
    ...
    if (req.body?.clearInput === true) {
      void sessionManager.clearInputAfterInterrupt(name)
        ...
        .catch(err => console.warn(`[stop-clear] ${name}: error — ${(err as Error).message}`));
    }
  ...
  router.post('/responses', (req, res) => {
    const { responseId, callingSession, targetSession, timeoutSeconds } = req.body as RegisterResponseRequest;
  ...
  router.post('/responses/:id', (req, res) => {
    ...
    const injectLate = (callingSession: string, expired: boolean) => {
