 ...
 */
export function chooseDetectionCandidate(candidates, opts) {
    const slack = opts.slackMs ?? RELINK_LIFETIME_SLACK_MS;
    const eligible = candidates.filter(c => !opts.claimedIds.has(c.sessionId) &&
 ...
 */
export function assessAssignedIdVerification(input) {
    if (input.idPresentInDir)
        return 'verified';
    ...
    if (input.failures >= minFailures && input.ageMs >= minAge)
        return 'fallback';
    return 'pending';
}
