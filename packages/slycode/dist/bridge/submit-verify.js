 ...
 */
export function extractInputRegion(provider, snapshot) {
    ...
    if (provider === 'claude') {
        ...
        for (let i = 0; i < lines.length; i++) {
            if (CLAUDE_SEPARATOR.test(lines[i]))
                sepIdx.push(i);
        }
        if (sepIdx.length < 2)
    ...
    if (provider === 'codex') {
        ...
        for (let i = promptIdx; i < lines.length; i++) {
            const line = lines[i];
            if (i > promptIdx && (line.trim() === '' || CODEX_FOOTER.test(line)))
                break;
            region.push(line);
        }
        const text = region.join('\n').replace(/[›»]/g, '');
...
function payloadQueued(regionText, expected) {
 ...
 */
export function countMatchingPlaceholders(snapshot, expected) {
 ...
 */
export function classifyInputRegion(provider, snapshot, expected) {
    ...
    if (provider === 'codex') {
        ...
        if (contentLines <= 1 && !parsePastePlaceholder(region.text))
            return 'empty';
    }
    return 'queued_other';
}
...
//# sourceMappingURL=submit-verify.js.map