 ...
 */
function mergeCardRefs(legacy, list) {
...
export class SlyActionFilter {
    ...
    buildCardContextHeader(context) {
        ...
        for (const ref of mergeCardRefs(card.design_ref, card.design_refs))
            lines.push(`Design Doc: ${ref}`);
        for (const ref of mergeCardRefs(card.feature_ref, card.feature_refs))
            lines.push(`Feature Spec: ${ref}`);
        for (const ref of mergeCardRefs(undefined, card.test_refs))
            lines.push(`Test Doc: ${ref}`);
