...
export function save(buffer, ext, contextSlug) {
    try {
        ...
        console.warn(`[audio-archive] save failed: ${err.message}`);
    }
}
export function prune(maxFiles) {
    try {
        const dir = resolveDir();
        ...
        for (const f of toRemove) {
            try {
                ...
                console.warn(`[audio-archive] prune unlink failed: ${err.message}`);
            }
        }
    }
    catch (err) {
        console.warn(`[audio-archive] prune failed: ${err.message}`);
    }
}
//# sourceMappingURL=audio-archive.js.map