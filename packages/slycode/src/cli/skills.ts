...
function parseSkillMeta(skillDir: string): SkillMeta | null {
  ...
  if (fmMatch) {
    ...
    if (uMatch) updated = uMatch[1].trim();
  }

  return { name, version, updated };
}

function getInstalledSkills(workspace: string): Map<string, SkillMeta> {
...
function getTemplateSkills(workspace: string): Map<string, SkillMeta> {
  const map = new Map<string, SkillMeta>();
...
async function listSkills(workspace: string): Promise<void> {
  ...
  const verWidth = 12;
  console.log(
    `  ${'Name'.padEnd(nameWidth)} ${'Installed'.padEnd(verWidth)} ${'Available'.padEnd(verWidth)} Status`
  );
  ...
  for (const name of [...allNames].sort()) {
    ...
    console.log(
      `  ${name.padEnd(nameWidth)} ${instVer.padEnd(verWidth)} ${tmplVer.padEnd(verWidth)} ${status}`
    );
  }
}

async function checkSkills(workspace: string): Promise<void> {
  const installed = getInstalledSkills(workspace);
  const templates = getTemplateSkills(workspace);
...
async function addSkill(workspace: string, name: string, all: boolean): Promise<void> {
  const packageDir = resolvePackageDir(workspace);
  if (!packageDir) {
...
async function resetSkill(workspace: string, name: string): Promise<void> {
  const packageDir = resolvePackageDir(workspace);
  if (!packageDir) {
    console.error('Could not find slycode package directory.');
    process.exit(1);
  ...
  if (fs.existsSync(dest)) {
    const ok = await confirm(
      `This will overwrite your customized "${name}" skill with the upstream version. Continue?`
    );
    if (!ok) {
