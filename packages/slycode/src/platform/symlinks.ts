...
function resolvePackageBin(workspace: string, tool: string): string {
  ...
  if (fs.existsSync(slycodePackageBin)) return slycodePackageBin;
  // Dev fallback
 ...
 */
export function linkClis(workspace: string): void {
 ...
 */
export function unlinkClis(): void {
  const binDir = getTargetBinDir();
