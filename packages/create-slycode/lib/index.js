...
function promptYN(rl, question, defaultYes = false) {
...
function writeEnvFile(dir, answers) {
    const lines = [
        '# ── Timezone ───────────────────────────────────────────────────────',
        ...
        '#',
        '# AWS Transcribe Streaming (set STT_BACKEND=aws-transcribe)',
...
function writeDefaultConfig(dir, answers) {
...
function copyTemplates(dir) {
    ...
    fs.writeFileSync(path.join(dir, 'data', 'commands.json'), JSON.stringify(commandsSeed, null, 2) + '\n');
    ...
    fs.writeFileSync(path.join(dir, 'projects', 'registry.json'), JSON.stringify(registrySeed, null, 2) + '\n');
 ...
 */
function findStoreTemplates(workspaceDir) {
...
function deployStoreAndSkills(dir) {
 ...
 */
function seedTutorialWorkspaceContent(dir) {
 ...
 */
function findTemplateFile(workspaceDir, relativePath) {
...
function deployUpdatesFolder(dir) {
