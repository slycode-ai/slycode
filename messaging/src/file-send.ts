...
export class FileSendError extends Error {
  constructor(public code: FileSendErrorCode, public httpStatus: number, message: string) {
    super(message);
...
export function mediaKindFromExtension(filePath: string): MediaKind | null {
  const ext = path.extname(filePath).toLowerCase();
  return EXT_TO_KIND[ext] ?? null;
}

export function resolveSendKind(kind: MediaKind | null, asOverride?: 'document'): SendKind | null {
  if (asOverride === 'document') return 'document';
  return kind;
}

export async function preflightFile(input: string, callerCwd?: string): Promise<{
  absolutePath: string;
  bytes: number;
  kind: MediaKind | null;
}> {
  ...
  try {
    ...
    if (code === 'ENOENT') throw new FileSendError('file_not_found', 404, `File not found: ${input}`);
    throw new FileSendError('file_unreadable', 400, `Cannot resolve path: ${(err as Error).message}`);
  }

  for (const pattern of SENSITIVE_PATH_PATTERNS) {
    if (pattern.test(absolutePath)) {
      throw new FileSendError(
        'denied_path',
        403,
        `Refusing to send: path matches sensitive-location guard (${pattern})`,
      );
    }
  }
  ...
  try {
    ...
    throw new FileSendError('file_unreadable', 400, `Cannot stat file: ${(err as Error).message}`);
  }
  if (!stat.isFile()) {
  ...
  if (stat.size > MAX_BYTES) {
    throw new FileSendError(
      'file_too_large',
      413,
      `File exceeds ${MAX_BYTES} bytes (got ${stat.size})`,
    );
  }
 ...
 */
export async function preflightWritePath(input: string): Promise<{ absolutePath: string }> {
  if (typeof input !== 'string' || input.length === 0) {
  ...
  for (const pattern of SENSITIVE_PATH_PATTERNS) {
    if (pattern.test(effectivePath)) {
      throw new FileSendError(
        'denied_path',
        403,
        `Refusing to write: path matches sensitive-location guard (${pattern})`,
      );
    }
  }
