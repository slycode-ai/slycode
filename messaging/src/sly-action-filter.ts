 ...
 */
function parseActionMd(content: string): { frontmatter: Record<string, unknown>; body: string } | null {
  ...
  for (const line of yamlStr.split('\n')) {
    ...
    if (currentKey && currentMap) { frontmatter[currentKey] = currentMap; currentMap = null; currentKey = null; }
    if (currentKey && currentArray) { frontmatter[currentKey] = currentArray; currentArray = null; currentKey = null; }
    ...
    if (!rawValue) {
      currentKey = key;
      if (key === 'classes') { currentMap = {}; } else { currentArray = []; }
      continue;
    }
 ...
 */
function scanActionsFromStore(): SlyActionsFile {
  const actionsDir = path.join(getWorkspaceRoot(), 'store', 'actions');
  const commands: Record<string, SlyActionConfig> = {};
  ...
  for (const entry of entries) {
    try {
      ...
      const classes = (typeof fm.classes === 'object' && fm.classes !== null && !Array.isArray(fm.classes))
        ? fm.classes as Record<string, number>
        : {};
...
export class SlyActionFilter {
  private actionsFile: SlyActionsFile | null = null;
  ...
  filterActions(
    ...
    placement?: 'startup' | 'toolbar',
    cardType?: string,
  ): Record<string, SlyActionConfig> {
    const file = this.loadActions();
    const result: Record<string, SlyActionConfig> = {};
