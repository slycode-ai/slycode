...
import { textToSpeech, convertToOgg, renderTtsAudio } from './tts.js';
...
interface ProviderDefaults {
  ...
  projects?: Record<string, { provider: string; model?: string }>;
}
...
function getProviderDefault(projectId?: string): { provider: string; model?: string } {
  try {
 ...
 */
function resolveKeysFor(projectIdOrProject: string | Project, state?: StateManager): string[] {
  if (typeof projectIdOrProject === 'string') {
    const proj = state?.getProjects().find(p => p.id === projectIdOrProject);
    if (!proj) return [projectIdOrProject];
...
function createChannel(state: StateManager): Channel | null {
...
async function renderSwitchView(
  ...
  // Cross-project recent-cards jump list — last row, global from anywhere.
  const recentRow: InlineButton[] = [{ label: '🕐 Recent', callbackData: 'sw_recent' }];
...
async function handleSessionLifecycle(
  ...
  opts: { suppressStartupActions?: boolean } = {},
): Promise<void> {
 ...
 */
function withTimestamp(content: string): string {
...
const renderQuickAccessRef: {
  value: (projectIds: string[], opts?: { silentIfEmpty?: boolean }) => Promise<boolean>;
} = { value: async () => false };
...
async function renderStartupActions(
  channel: Channel,
  state: StateManager,
  bridge: BridgeClient,
  kanban: KanbanClient,
...
function setupChannel(
  ...
  async function renderQuickAccess(
    projectIds: string[],
    opts: { silentIfEmpty?: boolean; maxTotal?: number; showMoreButton?: boolean; forceGlobal?: boolean } = {},
  ): Promise<boolean> {
  ...
  channel.onCommand('model', async () => {
    ...
    await channel.sendInlineKeyboard(
      `Model: *${currentLabel}* (${PROVIDER_LABELS[provider] || provider})`,
      buttons,
    );
  });
  ...
  channel.onCallback('prov_cur_', async (data) => {
    ...
    if (models.length > 0) {
      ...
      await channel.sendInlineKeyboard(
        `Provider set to: *${PROVIDER_LABELS[provider]}*\nSelect model:`,
        buttons,
      );
    } else {
...
async function main() {
   ...
   */
  async function maybeSendSwitchMessage(session: string | undefined): Promise<void> {
    if (!channel) return;
    if (!session || session === state.getSessionName()) return;
  ...
  app.post('/send/file', async (req, res) => {
    try {
      ...
      if (!channel) return res.status(400).json({ ok: false, error: 'no_channel', message: noChannelError });
      if (!channel.isReady()) return res.status(400).json({ ok: false, error: 'no_chat_configured', message: 'No active chat. Send a message from the channel first.' });
      ...
      const kind = resolveSendKind(preflight.kind, as as 'document' | undefined);
      if (kind === null) {
      ...
      let effectiveCaption: string | undefined = caption;
      ...
      try {
        ...
        return res.status(502).json({ ok: false, error: 'telegram_error', message: (err as Error).message });
      }
    } catch (err) {
  ...
  app.post('/tts/generate', async (req, res) => {
    try {
      ...
      const effectiveVoiceId: string | undefined =
        voiceId ?? state.resolveContextVoice({ projectId, session })?.id;
      ...
      try {
        ...
        return res.status(502).json({ ok: false, error: 'tts_failed', message: (err as Error).message });
      }
