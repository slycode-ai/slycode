...
export function CardModal({ card, stage, projectId, projectPath, onClose, onUpdate, onMove, onDelete, isCreateMode, onCreate, onAutomationToggle, suppressAutoTerminal }: CardModalProps) {
  ...
  // Terminal state (minimal - just for tab indicator)
  ...
  const hasHistory = sessionInfo?.hasHistory ?? false;
  ...
  // Fetch session info on mount for tab indicator and auto-switch.
  // Check all sessions for this project to find any provider's session for this card.
  useEffect(() => {
    ...
    fetch('/api/bridge/sessions')
      ...
      .then((data) => {
        ...
        // Find all sessions matching this card (any provider format)
        const matches = (data.sessions as SessionInfo[]).filter((s) =>
          s.name?.endsWith(cardSuffix) && s.name?.startsWith(`${projectId}:`)
        );
        if (matches.length === 0) return;
        // Prefer running > detached > stopped, then most recent
        const statusPriority = { running: 0, detached: 1, stopped: 2 };
        matches.sort((a, b) => {
        ...
        setSessionInfo({ status: match.status, hasHistory: match.hasHistory });
        // Extract provider from session name (format: projectId:provider:card:cardId)
        if (match.name) {
          const parts = match.name.split(':');
          const cardIdx = parts.indexOf('card');
          if (cardIdx === 2) {
            // New format: projectId:provider:card:cardId
            setDetectedProvider(parts[1]);
          }
        }
      })
      .catch(() => {
  ...
  // Auto-switch to terminal tab if session is running on initial load
  const [hasAutoSwitched, setHasAutoSwitched] = useState(false);
  useEffect(() => {
    if (!hasAutoSwitched && !suppressAutoTerminal && !isAutomation && sessionInfo && (sessionInfo.status === 'running' || sessionInfo.status === 'detached')) {
      // eslint-disable-next-line react-hooks/set-state-in-effect -- one-time auto-switch on session detection
      setActiveTab('terminal');
      setHasAutoSwitched(true);
    }
  }, [sessionInfo, hasAutoSwitched, suppressAutoTerminal]);
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Tabs */}
        <div className={`grain grain-soft flex overflow-x-auto scrollbar-hide ${modalStyles.tabsBorder} ${modalStyles.tabs}`}>
          <button
          ...
          >
            <div className={`h-2 w-2 rounded-full ${
              isRunning ? 'bg-green-500 shadow-[0_0_6px_rgba(34,197,94,0.6)]' : 'bg-void-400'
            }`} />
            Terminal
            {isRunning && (
          ...
          {activeTab === 'details' ? (
            ...
            <div className="h-full">
              <ClaudeTerminalPanel
                ...
                initialProvider={detectedProvider ?? undefined}
                footerClassName={terminalColor}
                tintColor={terminalTint}
                onSessionChange={(info) => setSessionInfo(info ? { status: info.status } : null)}
                onProviderChange={(provider) => setDetectedProvider(provider)}
